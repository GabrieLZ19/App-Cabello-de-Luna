...
import { View, Text, TouchableOpacity, ScrollView, Image, TextInput, StatusBar, ActivityIndicator, KeyboardAvoidingView, Platform } from 'react-native';
import { SafeAreaView } from 'react-native-safe-area-context';
import { useRouter } from 'expo-router';
import { User, Mail, Ticket, Lock, Eye, EyeOff, ArrowRight, ShieldCheck, AlertCircle } from 'lucide-react-native';
...
export default function RegisterScreen() {
  ...
  const handleRegister = async () => {
    if (!code.trim()) {
      setError('El código de inscripción es obligatorio.');
      return;
    }
    ...
    try {
      // 1. Validar franquicia en tiempo real con Supabase
      await validateFranchiseCode(code.trim());

      // 2. Registrar estudiante en Supabase PostgreSQL
      ...
      // 3. Redirigir a verificación de cuenta (04_VerificarCuenta.png)
      router.push({
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#0C0A07' }}>
      ...
      >
        ...
        >
          {/* Header Center Logo matching 02b_Registro.png */}
          <View style={{ alignItems: 'center', marginBottom: 24 }}>
            <Image
              ...
              resizeMode="contain"
            />
            <Text style={{ color: '#FFFFFF', fontSize: 26, fontWeight: 'bold', textAlign: 'center', marginBottom: 6 }}>
              Activá tu inscripción
            </Text>
            <Text style={{ color: '#B0A894', fontSize: 14, textAlign: 'center' }}>
              Creá tu cuenta con el código de tu instituto.
            </Text>
          </View>
          ...
          {/* Form Fields matching 02b_Registro.png with active focus styling */}
          <View style={{ marginBottom: 20 }}>
            ...
            <View style={{ marginBottom: 16 }}>
              ...
              <View style={getInputStyle('fullName')}>
                <User color={focusedInput === 'fullName' ? '#C9A45C' : '#897F6B'} size={18} />
                <TextInput
            ...
            <View style={{ marginBottom: 16 }}>
              ...
              <View style={getInputStyle('email')}>
                <Mail color={focusedInput === 'email' ? '#C9A45C' : '#897F6B'} size={18} />
                <TextInput
            ...
            {/* Franchise / Enrollment Code */}
            <View style={{ marginBottom: 16 }}>
              <Text style={{ color: '#B0A894', fontSize: 12, fontWeight: '600', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8 }}>
                CÓDIGO DE INSCRIPCIÓN
              </Text>
              <View style={getInputStyle('code')}>
                <Ticket color={focusedInput === 'code' ? '#C9A45C' : '#897F6B'} size={18} />
                <TextInput
                  ...
                  style={{ color: '#FFFFFF', flex: 1, fontSize: 15, fontWeight: 'bold', marginLeft: 12 }}
                />
              </View>
            </View>
          ...
          {/* Submit Button matching 02b_Registro.png */}
          <TouchableOpacity
          ...
          <View style={{ alignItems: 'center', marginBottom: 20 }}>
            <TouchableOpacity onPress={() => router.push('/auth/login')} activeOpacity={0.7} style={{ flexDirection: 'row' }}>
              <Text style={{ color: '#B0A894', fontSize: 13 }}>¿Ya tenés una cuenta? </Text>
              <Text style={{ color: '#C9A45C', fontSize: 13, fontWeight: 'bold' }}>Ingresar</Text>
            </TouchableOpacity>
          </View>

          {/* Info Box matching 02b_Registro.png */}
          <View style={{ backgroundColor: '#15100A', borderWidth: 1, borderColor: 'rgba(255, 255, 255, 0.1)', borderRadius: 14, padding: 16, flexDirection: 'row', alignItems: 'flex-start' }}>
            <ShieldCheck color="#C9A45C" size={20} style={{ marginTop: 2, marginRight: 10 }} />
            <Text style={{ color: '#B0A894', fontSize: 12, lineHeight: 18, flex: 1 }}>
              El código de inscripción lo emite tu instituto franquiciado. Sin código no podés registrarte.
            </Text>
          </View>
        </ScrollView>
      </KeyboardAvoidingView>
