...
import { KeyRound, Mail, ChevronLeft, Send, ArrowLeft, AlertCircle } from 'lucide-react-native';
...
export default function ForgotPasswordScreen() {
  ...
  const handleSendResetCode = async () => {
    ...
    try {
      // Solicitar código OTP real al backend NestJS
      await requestPasswordReset(email.trim());

      // Redirigir a la pantalla de verificación OTP (04_VerificarCuenta.png)
      router.push({
        pathname: '/auth/verify',
        params: { email: email.trim(), mode: 'reset-password' },
      });
    } catch (err: any) {
      setError(err.message || 'Error al procesar la solicitud con el servidor.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#0C0A07' }}>
      ...
      >
        ...
        >
          {/* Top Left Circular Back Arrow Button matching 03_RecuperarPassword.png */}
          <TouchableOpacity
            onPress={() => router.back()}
            activeOpacity={0.7}
            style={{
          ...
          {/* Key Icon Box matching 03_RecuperarPassword.png */}
          <View
            style={{
              width: 60,
              height: 60,
          ...
          {/* Header Text matching 03_RecuperarPassword.png */}
          ...
          {/* Email Input Field matching 03_RecuperarPassword.png */}
          ...
          {/* Primary Action Button matching 03_RecuperarPassword.png */}
          <TouchableOpacity
          ...
          >
            {loading ? (
              ...
              <>
                ...
                <Text style={{ color: '#0C0A07', fontWeight: 'bold', fontSize: 16 }}>
                  Enviar enlace
                </Text>
              </>
            )}
          ...
          {/* Back to Login Link matching 03_RecuperarPassword.png */}
