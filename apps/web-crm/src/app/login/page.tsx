...
export default function LoginPage() {
  ...
  const [selectedRole, setSelectedRole] = useState<"ADMIN" | "ASSISTANT">(
    "ADMIN",
  );
  ...
  return (
    <div className="min-h-screen bg-[#0C0A07] flex items-center justify-center p-4 sm:p-6">
      <div className="bg-[#15100A] border border-[#C9A45C]/30 rounded-2xl sm:rounded-3xl p-6 sm:p-8 w-full max-w-md shadow-2xl space-y-6">
        {/* Brand Header */}
        <div className="text-center space-y-2">
          <img
            src="/logo.png"
            alt="Método Cabello de Luna"
            className="w-14 h-14 sm:w-16 sm:h-16 rounded-full border-2 border-[#C9A45C] object-cover mx-auto shadow-xl"
          />
          <h1 className="text-xl sm:text-2xl font-bold text-white">
            Centro de Control CRM
          </h1>
          <p className="text-[10px] sm:text-xs text-[#C9A45C] uppercase tracking-widest font-semibold">
        ...
        <div className="bg-[#1A140E] p-3 rounded-2xl border border-white/10 space-y-2">
          <p className="text-[10px] sm:text-xs font-bold text-[#B0A894] uppercase tracking-wider text-center">
          ...
          <div className="grid grid-cols-2 gap-2">
            <button
              ...
              className={`p-2.5 sm:p-3 rounded-xl border text-[11px] sm:text-xs font-bold transition-all flex flex-col items-center gap-1 ${
                selectedRole === "ADMIN"
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs font-bold text-[#B0A894] uppercase mb-1">
              Correo Electrónico
            </label>
            <input
              type="email"
